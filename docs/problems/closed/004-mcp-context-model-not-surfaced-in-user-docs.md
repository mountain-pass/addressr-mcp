...
- [x] Implement fix in README.md (2026-05-13, folded into the fix commit per ADR-022)
- [x] Verify by re-reading the README as a first-time adopter (2026-05-13: VERIFIED by user; conditional on em-dash cleanup which landed in the closure commit)
...
Closed 2026-05-13 on explicit user confirmation after re-reading the new `## How It Works` section. User flagged a follow-up: the section (and a handful of other lines in the README) contained em-dashes, which the user does not want in their docs. Em-dashes were replaced with hyphens, commas, semicolons, or rephrased in this same closure commit. User-visible behavioural target of P004 met: a first-time adopter can now answer "how does this stay cheap?" from the README without inspecting `src/server.mjs`.

The em-dash preference is captured as a project-level feedback memory at `.claude/projects/-Users-tomhoward-Projects-addressr-mcp/memory/feedback_no_em_dashes.md` and will apply to future assistant-authored prose.
